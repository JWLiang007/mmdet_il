...
class M_DIFGSM_BG_TGT(Attack):
    ...
    def loss_cls_hook(self,):
        def pre_forward_hook(module, args):
            labels = args[1][0]
            labels[labels!=torch.max(labels)] = torch.max(labels)

        self.model.module.bbox_head.loss_cls.register_forward_pre_hook(pre_forward_hook)
    ...
    def forward(self, data):
        ...
        for i in range(self.steps):
            ...
            # target attack: minimize the loss_cls to force the model predict bg cat
            loss_cls.backward()
