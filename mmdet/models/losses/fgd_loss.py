...
class FGDLoss(nn.Module):
    ...
    def get_fea_loss(self, preds_S, preds_T, Mask_fg, Mask_bg, C_s, C_t, S_s, S_t):
        ...
        Mask_bg = torch.sqrt(Mask_bg)
        # Mask_bg[Mask_bg==0] = torch.sign(fea_t[Mask_bg==0])  * (-1e6)
        bg_fea_t = torch.mul(fea_t, Mask_bg)
        bg_fea_t =  [bg_fea_t[i,:,Mask_bg[i,0]!=0] for i in range(N)]
        ...
        bg_fea_s = [bg_fea_s[i,:,Mask_bg[i,0]!=0] for i in range(N)]
